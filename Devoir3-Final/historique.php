...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	if(strcmp($_SESSION["Role"],"G")==0){	
		...
		echo "<legend>Verifier historique d'un adherent</legend>";
            ...
            echo "<br/><label> Consulter tout les reservations: </label><br/>";
            echo "<input type='submit' name='subT' value='Voir tout'/>";
			echo "</fieldset>";
	}			
	?>	
	</form>
	...
	if(strcmp($_SESSION["Role"],"G")==0){	
		if($_SERVER['REQUEST_METHOD']=='POST'){
            if(isset($_POST['subT'])){
				$histData = mysqli_query($conn,"select * from reservation")or die(mysqli_error($conn));
				if($histData->num_rows == 0){
					echo "<span>Aucun resultat!</span>";
				}				
				$r = 1;
				while($rangeData = mysqli_fetch_assoc($histData)){
					$chTer = mysqli_query($conn,"select Nom_ter from terrain where ID_T='".$rangeData['ID_RT']."'")or die(mysqli_error($conn));
					$resTer = mysqli_fetch_assoc($chTer);
					$nomTer= $resTer ['Nom_ter'];
					$chAdh = mysqli_query($conn,"select Prenom, Nom from adherents where ID_A='".$rangeData['ID_RA']."'")or die(mysqli_error($conn));
					$resAdh = mysqli_fetch_assoc($chAdh);
					$nomAdherent= $resAdh ['Nom'];
					$prenomAdherent= $resAdh ['Prenom'];
					echo "<tr><td>".$r++."</td><td>" .$prenomAdherent." ".$nomAdherent."</td><td>".$nomTer."</td><td>".$rangeData['Date_R']."</td><td>".$rangeData['Hour_R']."</td><td>".$rangeData['Date_RD']."</td></tr>";
				} 
            }else{
				$adh=($_POST['adherent']);
