...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body id="G">
    	<header class="headerAdmin">
    		<?php
			// C'est la version d'admin pour la partie de reservation.
...
<!-- SECTION DE LA VERFICATION DES TERRAINS DISPONIBLES ................... -->
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		if(strcmp(date('l'),"Tuesday")!=0){ //--------------------------------------------------------------------------------------------------------CHANGE DAY
			...
			$dateOfTmr= date('Y-m-d', strtotime($dateOfToday. ' + 1 days'));
			if($ish>$ieh){
				echo "<script>alert('L`heure de debut doit etre inferieur que celle de fin!'); parent.location='admin_reserve.php'</script>";
			}else{
                $ID_TArr = array(1,2,3,4,5,6);
                echo "<table border='1'>";
                echo "<tr><th>Nom de terrain</th><th>Disponibilit&eacute;s</th></tr>";          
                for($i=$ish; $i<$ieh; $i++){
                    $j=$i+1;
                    $hourR = $i."h - ".$j."h";
                    for($k=0; $k<6 ; $k++){
                        $idTer = $ID_TArr[$k];
                        $resInfo = mysqli_query($conn,"select * from reservation where ID_RT='".$idTer."' AND Hour_R='".$hourR."' AND Date_R='".$dateOfTmr."'")or die(mysqli_error($conn));
                        if((mysqli_num_rows($resInfo)==0)){
                            $terInfo = mysqli_query($conn,"select * from terrain where ID_T='".$idTer."'")or die(mysqli_error($conn));
                            $terRange = mysqli_fetch_assoc($terInfo);
                            $terNom = $terRange['Nom_ter'];                            
                                echo "<tr><td>$terNom</td><td>$dateOfTmr &agrave; $hourR</td></tr>";                           
                            }
                        }
                    }
                    echo "</table>";
...
<br/>
	<b><big>NOTE:</big> Une reservation est permit seulement pendant les Mardis pour reserver un terrain le lendeman soit Mercredi.</b>
	</fieldset>
	</form>
...
<form id="cancel" method="POST" action="<?PHP $_PHP_SELF ?>">
	<fieldset>
		...
		<table border="1">
			<thead>Les reservations currentes:</thead>
	...
	//Verfier le jour ..............................................................................................................................................................		
			if(strcmp($currentDay,"Tuesday")!=0){
				echo "<script>alert('Vous ne pouvez reserver un terrain qu`aux mardis!'); parent.location='admin_reserve.php'</script>";
